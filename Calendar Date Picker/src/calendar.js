...
document.addEventListener("click", function(e) {
    if(e.target.closest('#calendar')) {
        // 캘린더 내부를 클릭됐을 경우
        // DO_NOTHING
    } else if (e.target == selectInput) {
        // input 요소가 클릭됐을 경우
        divCalendar.classList.toggle('is-hidden');
    } else {
        divCalendar.classList.add('is-hidden');
    }
});
...
function showDate(e) {
  ...
  let showNumDay = String(showDay).padStart(2, '0');
  let showNumMonth =  String(months.indexOf(months[showMonth]) + 1).padStart(2, '0');
  document.getElementById("select").value =
    showYear + "-" + showNumMonth + "-" + showNumDay;
    console.log(showYear + "-" + showNumMonth + "-" + showNumDay);
}
